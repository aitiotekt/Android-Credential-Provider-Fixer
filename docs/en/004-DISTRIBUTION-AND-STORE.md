...
Version `0.1.0-alpha.3` has CI build verification only. There is no release workflow, signing secret interface, notarization job, documentation deployment, or published installer. These are Phase 4 tasks and must not be represented as complete before signed artifacts are manually validated.
