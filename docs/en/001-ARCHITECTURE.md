...
The frontend owns rendering, opaque candidate/device selection, confirmation, localization, and accessible interaction. It never parses ADB output. IPC endpoints cover startup state, ADB discovery/selection, native ADB selection, device listing/inspection, onboarding, and the deterministic Demo fixture. Native paths and serials stay in backend session state. Demo data uses the same report DTO but cannot call the live adapter.
