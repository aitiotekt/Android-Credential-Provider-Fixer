...
> [!IMPORTANT]
> 当前 `0.1.0-alpha.1` 是工程基线，只包含应用外壳、共享 core 契约、进程适配器、文档和测试，**不会**运行 ADB 诊断或修改设备。

## 项目背景

在部分 Android 14 及以上设备中，OEM 设置界面可能显示某个密码管理器为首选，但该 provider 没有进入 Credential Manager 实际使用的 enabled 状态。项目后续会把这种不一致明确展示出来，在修改前生成计划并保存原状态，对每次写入进行回读验证，并提供恢复能力。
...
项目通过 [mise](https://mise.jdx.dev/) 管理 Node 26.1.0、pnpm 11.1.2、Rust 1.98.0、Just 和 prek。
...
当前桌面构建只暴露安全的 `get_app_info` IPC；前端没有 shell execute 或 spawn 权限。
