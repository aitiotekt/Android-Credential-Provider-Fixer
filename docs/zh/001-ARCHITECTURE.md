...
前端负责展示、不透明 candidate/device 选择、确认、本地化与无障碍交互，不解析 ADB 输出。IPC 仅覆盖启动状态、ADB 发现/选择、原生 ADB 文件选择、设备列表/检查、onboarding 和确定性 Demo fixture。原生路径与 serial 保留在后端 session 中；Demo 使用同一报告 DTO，但不能调用真实 adapter。
