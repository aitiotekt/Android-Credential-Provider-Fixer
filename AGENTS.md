...
- Organize recipes by responsibility: `justfiles/dev.just` for development entry points, `build.just` for builds/staging, `quality.just` for checks/verification, `test.just` for tests, `setup.just` for dependencies/toolchain setup, and `maintenance.just` for synchronization and version/release-policy changes. Keep existing public recipe names and behavior stable when relocating them.
- For long-running development commands that forward arbitrary arguments, prefer `[unix]` / `[windows]` recipes over a Node launcher subprocess. Use POSIX `exec` with `"$@"` or a PowerShell script with `@args` and explicit native exit-code propagation; never interpolate forwarded arguments into shell source. Keep terminal I/O inherited and verify forwarding in isolated fixtures on each platform.
...
- `just dev-android` is an explicit developer-only exception for installing and launching this repository's debug app on a selected running device. Its orchestration belongs to `scripts/dev-cli.mts`, not the public Rust CLI or app runtime. Always require device selection, pin the transport and foreground user across builds, and never uninstall, clear data, change settings or manage ADB servers/emulators. `--watch` means serialized rebuild/reinstall/relaunch, not state-preserving hot reload; test it with scripted runners only. Just argument forwarding must be tested in an isolated fixture before running a real development entry point.
